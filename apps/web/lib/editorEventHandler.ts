...
export async function step(
  kernel: Kernel,
  interacter: EditorInteracter,
): Promise<Result<StepResult, string>> {
  if (!interacter) {
    return { tag: "Err", error: "No editor interacter." };
  }

  const content = interacter.getMainEditorContent();
  const commandsResult: Result<CmdWithLoc[], PartialProgram> =
    kernel.parse(content);

  const commands =
    commandsResult.tag === "Ok"
      ? commandsResult.value
      : commandsResult.error.cmds;
...
export function undo(
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (result.tag === "Err") {
      ...
      interacter.setGoalEditorContent(
        formatProofState(kernel.getCurrentGoals()),
      );
    }
  }
